...
import { TRANSLATE_LANGUAGES } from '@/lib/editor/translate-languages'
import { helloForLanguage } from '@/lib/editor/language-hellos'
...
import { ToggleSwitch } from './selection-ai-toggles'
...
import { useSuggestBlock, SUGGEST_OPTIONS, type SuggestMenuKind } from './use-suggest-block'
import { useAiTools } from './use-ai-tools'
import { useFindIssuesPanel } from './use-find-issues-panel'
import { useSummarizeFlow, type TextRange } from './use-summarize-flow'
import { ToolbarPopover } from '../toolbar-popover'
import { UPCOMING_FEATURE_GROUPS } from './upcoming-features'
import { useAnalyticsTools } from './use-analytics-tools'
import {
  CHART_TYPE_OPTIONS,
...
function shouldShowAiSelection({ editor: ed }: { editor: Editor }) {
  // Image AI lives on ImageMenu — avoid overlapping bubble menus
...
export function AiSelectionMenu({ editor }: AiSelectionMenuProps) {
  ...
  const [, setBusyLabel] = useState('Working…')
  ...
  const confirmActive = useCallback(() => {
    ...
    // Stay open for undo window — don't clearUi
  ...
  const runSuggest = useCallback((kind: SuggestMenuKind) => {
    setPanel('main')
    setBusyLabel(kind === 'auto' ? 'Structuring…' : 'Suggesting…')
  ...
  const runExplain = useCallback((audience: ExplainAudience) => {
    ...
    if (!text.trim()) return
    setPanel('main')
    const label = audience === 'explain' ? 'Explaining…' : 'Rewriting…'
    setBusyLabel(label)
  ...
  const runActionItems = useCallback(() => {
    setBusyLabel('Extracting actions…')
  ...
  const runTranslate = useCallback((lang: string) => {
    ...
    void rewrite.run(text, range, (signal) => aiApi.translate({ text, targetLang: lang }, signal), 'Translating…')
  ...
  const runTone = useCallback((prompt: string) => {
    ...
    void rewrite.run(text, range, (signal) => aiApi.customTone({ selection: text, tone: prompt }, signal), 'Rewriting…')
  }, [selectionRange, captureRange, editor, rewrite])
  ...
  const runBasics = useCallback(async (mode: 'spelling' | 'grammar') => {
    ...
    // In-place fix only — no full-block regeneration / Confirm bar
      ...
      >
        {panel === 'summarize' ? (
          <div className="flex flex-col w-40 rounded-xl border border-border bg-popover shadow-xl p-1">
            ...
            {LENGTHS.map((l) => (
              <MenuRow key={l.value} onClick={() => generateSummary(l.value)} title={`Summarize — ${l.label}`}>
          ...
          >
            ...
            <div className="p-3 space-y-2">
              <p className="text-[11px] text-muted-foreground leading-relaxed">
                Paste requirements the draft must cover (PPE, lift plan, weather…).
              </p>
              <textarea
                value={briefText}
          ...
          <div className="flex w-72 min-w-[17.5rem] flex-col gap-1.5" onMouseDown={(e) => e.preventDefault()}>
            ...
            <AiToggleRow
              icon={Zap}
              label="Autocomplete"
              title="Autocomplete — ghost text while typing (Tab to accept)"
